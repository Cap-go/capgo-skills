...
description: Guides the agent through an Apple App Store preflight review for Capacitor apps before submission or after rejection. Covers guideline checklist selection, App Store metadata review, Capacitor/iOS project inspection, privacy manifests, Sign in with Apple, entitlements, and common rejection patterns. Do not use for Google Play review, generic store publishing only, or non-Apple mobile runtimes.
allowed-tools:
  - Bash(node -e *)
  - Bash(find *)
  - Bash(rg *)
...
This skill wraps the Apple guideline and rejection-rule corpus adapted from [truongduy2611/app-store-preflight-skills](https://github.com/truongduy2611/app-store-preflight-skills) and adds Capacitor-specific project inspection.
...
- User wants a review-notes checklist for a Capacitor iOS submission

## Live Project Snapshot

Detected Capacitor, iOS, auth, subscription, and privacy-related packages:
!`node -e "const fs=require('fs');if(!fs.existsSync('package.json'))process.exit(0);const pkg=JSON.parse(fs.readFileSync('package.json','utf8'));const matchers=['@capacitor/core','@capacitor/ios','@capacitor/app','@capacitor/browser','@capacitor/preferences','@capacitor/push-notifications','@capacitor/filesystem','@capacitor/camera','@capacitor/geolocation','@capacitor/share','@capgo/capacitor-social-login','@codetrix-studio/capacitor-google-auth'];const out=[];for(const section of ['dependencies','devDependencies']){for(const [name,version] of Object.entries(pkg[section]||{})){if(matchers.includes(name)||name.includes('firebase')||name.includes('sentry')||name.includes('stripe')||name.includes('revenuecat')||name.includes('purchase')||name.includes('subscription'))out.push(section+'.'+name+'='+version)}}console.log(out.sort().join('\n'))"`

Relevant Apple review file paths:
!`find . -maxdepth 5 \( -name 'capacitor.config.json' -o -name 'capacitor.config.ts' -o -name 'capacitor.config.js' -o -name 'Info.plist' -o -name '*.entitlements' -o -name 'PrivacyInfo.xcprivacy' -o -name 'project.pbxproj' -o -path './ios' -o -path './fastlane/metadata' -o -path './metadata' \)`
...
Use `references/guidelines/README.md` for the complete guideline index when a rejection cites a specific section.
...
- `capacitor.config.*` for app identifiers and configuration
...
- missing or vague usage strings for camera, photo library, microphone, location, notifications, or tracking
...
If App Store metadata is not stored locally and `asc` is available, pull it. If not, audit the local metadata sources and tell the user what remains unverified.
...
### Step 6: Suggest Review Notes and Submission Follow-Ups
...
- If the app metadata is unavailable locally and `asc` is not configured, continue with the code and project audit and clearly mark metadata checks as partial.
- If the project is mostly a thin web wrapper, call out guideline 4.2 risk directly rather than burying it in warnings.
